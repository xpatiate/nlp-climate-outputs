...
It was generated using GPT model 3.5 via langchain, using the [map-reduce QA chain](https://github.com/xpatiate/nlp-climate-outputs), with 4000-character chunks of context and customised question and combine prompts. Text from the whole policy document is divided into 4000-character excerpts and stored. For each question, the system selects the four most relevant excerpts and asks GPT the question in relation to each one. GPT returns four answers, including specific quotes from the excerpt, which are then combined to produce the overall answer.
...
In this output document, each question is displayed along with the four context excerpts and the answers returned by GPT, followed by the final answer. Only the first few characters of each context chunk are displayed, as the original text can be found in the policy document.
